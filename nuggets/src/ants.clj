...
(def world
  (apply vector
         (map (fn [_]
                (apply vector (map (fn [_] (ref (Cell. 0 0 nil nil)))
                                   (range dim))))
              (range dim))))
    ...
    (loop [i 0 sum 0]
      (if (< r (+ (slices i) sum))
        ...
        (recur (inc i) (+ (slices i) sum))))))
                                        ...
                                        ;
(def dir-delta
  "dirs are 0-7, starting at north and going clockwise these are the
  deltas in order to move one step in given dir."
  {0 [0 -1]
   1 [1 -1]
   2 [1 0]
   3 [1 1]
...
(defn animation [x ^JPanel panel]
  (when running
    (send-off *agent* #'animation panel))
  (. panel (repaint))
...
(comment
  ...
  (send-off animator animation panel)
